...
export function ShareCatalogDialog() {
  ...
  const basicLink = `${baseUrl}/catalog/${user.id}/basic`;
  ...
  const shareOnWhatsApp = (link: string, catalogType: string) => {
    ...
    if (catalogType === 'public') {
      message = `Check out our latest inventory catalog with prices:\n${link}`;
    } else if (catalogType === 'basic') {
      message = `View our product catalog (item names and codes only):\n${link}`;
    } else {
      message = `Here's our inventory catalog with current stock levels:\n${link}`;
    }
    ...
    <Dialog>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <Label>Basic Catalog (No Prices)</Label>
            <p className="text-sm text-muted-foreground mb-2">
              Share with customers - shows only item names, codes, and categories (no prices)
            </p>
            <div className="flex gap-2">
              <Input
                value={basicLink}
              ...
              <Button
                size="icon"
                variant="outline"
                onClick={() => copyToClipboard(basicLink, "basic")}
                title="Copy link"
              >
                {copied === "basic" ? (
                  <Check className="w-4 h-4 text-green-600" />
              ...
              <Button
                size="icon"
                variant="outline"
                onClick={() => shareOnWhatsApp(basicLink, 'basic')}
                className="bg-[#25D366] hover:bg-[#20BA5A] text-white border-[#25D366]"
                title="Share on WhatsApp"
              >
                <svg
